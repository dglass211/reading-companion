import React from 'react';
import Svg, { Path } from 'react-native-svg';

interface IconProps {
  size?: number;
  color?: string;
}

export const IconNotes: React.FC<IconProps> = ({ size = 24, color = '#6A6B6A' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path
        d="M9 7H15M9 11H15M9 15H13M5 5C5 4.46957 5.21071 3.96086 5.58579 3.58579C5.96086 3.21071 6.46957 3 7 3H17C17.5304 3 18.0391 3.21071 18.4142 3.58579C18.7893 3.96086 19 4.46957 19 5V19C19 19.5304 18.7893 20.0391 18.4142 20.4142C18.0391 20.7893 17.5304 21 17 21H7C6.46957 21 5.96086 20.7893 5.58579 20.4142C5.21071 20.0391 5 19.5304 5 19V5Z"
        stroke={color}
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

export const IconHome: React.FC<IconProps> = ({ size = 24, color = '#6A6B6A' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path
        d="M5 12H3L12 3L21 12H19M5 12V19C5 19.5304 5.21071 20.0391 5.58579 20.4142C5.96086 20.7893 6.46957 21 7 21H17C17.5304 21 18.0391 20.7893 18.4142 20.4142C18.7893 20.0391 19 19.5304 19 19V12"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M9 21V15C9 14.4696 9.21071 13.9609 9.58579 13.5858C9.96086 13.2107 10.4696 13 11 13H13C13.5304 13 14.0391 13.2107 14.4142 13.5858C14.7893 13.9609 15 14.4696 15 15V21"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

export const IconBooks: React.FC<IconProps> = ({ size = 24, color = '#2D2E2D' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path
        d="M9 5C9 4.73478 8.89464 4.48043 8.70711 4.29289C8.51957 4.10536 8.26522 4 8 4H6C5.73478 4 5.48043 4.10536 5.29289 4.29289C5.10536 4.48043 5 4.73478 5 5V19C5 19.2652 5.10536 19.5196 5.29289 19.7071C5.48043 19.8946 5.73478 20 6 20H8C8.26522 20 8.51957 19.8946 8.70711 19.7071C8.89464 19.5196 9 19.2652 9 19M9 5V19M9 5C9 4.73478 9.10536 4.48043 9.29289 4.29289C9.48043 4.10536 9.73478 4 10 4H12C12.2652 4 12.5196 4.10536 12.7071 4.29289C12.8946 4.48043 13 4.73478 13 5V19C13 19.2652 12.8946 19.5196 12.7071 19.7071C12.5196 19.8946 12.2652 20 12 20H10C9.73478 20 9.48043 19.8946 9.29289 19.7071C9.10536 19.5196 9 19.2652 9 19M5 8H9M9 16H13"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M14.0001 9.00004L18.0001 8.00004M16.0001 16L19.9231 15.02M13.8031 4.56004L15.9871 4.03004C16.5491 3.89504 17.1201 4.22004 17.2691 4.76204L20.9641 18.18C21.0293 18.4257 21.0004 18.6868 20.8831 18.9123C20.7659 19.1378 20.5686 19.3114 20.3301 19.399L20.1971 19.44L18.0131 19.97C17.4511 20.105 16.8801 19.78 16.7311 19.238L13.0361 5.82004C12.9709 5.57439 12.9998 5.31325 13.117 5.08778C13.2343 4.8623 13.4315 4.68872 13.6701 4.60104L13.8031 4.56004Z"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

export const IconMic: React.FC<IconProps> = ({ size = 24, color = '#6A6B6A' }) => {
  return (
    <Svg width={size} height={(size * 26) / 28} viewBox="0 0 28 26" fill="none">
      <Path d="M14 3C15.2598 3 16.468 3.49761 17.3588 4.38337C18.2496 5.26912 18.75 6.47046 18.75 7.72311V13.627C18.75 14.8796 18.2496 16.081 17.3588 16.9667C16.468 17.8525 15.2598 18.3501 14 18.3501C12.7402 18.3501 11.532 17.8525 10.6412 16.9667C9.75044 16.081 9.25 14.8796 9.25 13.627V7.72311C9.25 6.47046 9.75044 5.26912 10.6412 4.38337C11.532 3.49761 12.7402 3 14 3Z" fill={color} />
      <Path d="M22.3125 12.4462C22.6274 12.4462 22.9295 12.5706 23.1522 12.7921C23.3749 13.0135 23.5 13.3138 23.5 13.627C23.5003 15.9278 22.656 18.1495 21.1257 19.8753C19.5955 21.601 17.4844 22.7121 15.1887 23H12.8125C10.5166 22.7124 8.40522 21.6014 6.87471 19.8756C5.34419 18.1498 4.49978 15.9279 4.5 13.627C4.5 13.3138 4.62511 13.0135 4.84781 12.7921C5.07051 12.5706 5.37256 12.4462 5.6875 12.4462C6.00244 12.4462 6.30449 12.5706 6.52719 12.7921C6.74989 13.0135 6.875 13.3138 6.875 13.627C6.875 15.506 7.62567 17.308 8.96186 18.6366C10.2981 19.9652 12.1103 20.7117 14 20.7117C15.8897 20.7117 17.7019 19.9652 19.0381 18.6366C20.3743 17.308 21.125 15.506 21.125 13.627C21.125 13.3138 21.2501 13.0135 21.4728 12.7921C21.6955 12.5706 21.9976 12.4462 22.3125 12.4462Z" fill={color} />
    </Svg>
  );
};

export const IconPlus: React.FC<IconProps> = ({ size = 24, color = '#66A0C8' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path d="M12 5V19M5 12H19" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </Svg>
  );
};

export const IconReplace: React.FC<IconProps> = ({ size = 24, color = '#66A0C8' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path d="M21 11V8C21 7.46957 20.7893 6.96086 20.4142 6.58579C20.0391 6.21071 19.5304 6 19 6H13M13 6L16 9M13 6L16 3M3 13V16C3 16.5304 3.21071 17.0391 3.58579 17.4142C3.96086 17.7893 4.46957 18 5 18H11M11 18L8 15M11 18L8 21M3 4C3 3.73478 3.10536 3.48043 3.29289 3.29289C3.48043 3.10536 3.73478 3 4 3H8C8.26522 3 8.51957 3.10536 8.70711 3.29289C8.89464 3.48043 9 3.73478 9 4V8C9 8.26522 8.89464 8.51957 8.70711 8.70711C8.51957 8.89464 8.26522 9 8 9H4C3.73478 9 3.48043 8.89464 3.29289 8.70711C3.10536 8.51957 3 8.26522 3 8V4ZM15 16C15 15.7348 15.1054 15.4804 15.2929 15.2929C15.4804 15.1054 15.7348 15 16 15H20C20.2652 15 20.5196 15.1054 20.7071 15.2929C20.8946 15.4804 21 15.7348 21 16V20C21 20.2652 20.8946 20.5196 20.7071 20.7071C20.5196 20.8946 20.2652 21 20 21H16C15.7348 21 15.4804 20.8946 15.2929 20.7071C15.1054 20.5196 15 20.2652 15 20V16Z" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </Svg>
  );
};

export const IconBack: React.FC<IconProps> = ({ size = 24, color = '#66A0C8' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path d="M15 6L9 12L15 18" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </Svg>
  );
};

export const IconRefresh: React.FC<IconProps> = ({ size = 24, color = '#6BA7D1' }) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path d="M20 11.0002C19.7554 9.24041 18.9391 7.60985 17.6766 6.35969C16.4142 5.10953 14.7758 4.30911 13.0137 4.08175C11.2516 3.85438 9.46362 4.21268 7.9252 5.10144C6.38678 5.9902 5.18325 7.36013 4.5 9.00019M4 5.00019V9.00019H8M4 13.0002C4.24456 14.76 5.06093 16.3905 6.32336 17.6407C7.58579 18.8909 9.22424 19.6913 10.9863 19.9186C12.7484 20.146 14.5364 19.7877 16.0748 18.8989C17.6132 18.0102 18.8168 16.6403 19.5 15.0002M20 19.0002V15.0002H16" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </Svg>
  );
};
